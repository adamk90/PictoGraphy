...
public:
    Ciff(): headerSize(0), contentSize(0), width(0), height(0), caption(""), tags(vector<string>{}), pixels(vector<Pixel>{}) {};
    
    //Magic + Header + Content + Width + Height + '\n' + '\0'
    static const int MIN_HEADER_SIZE = 4 + 8 + 8 + 8 + 8 + 1 + 1;
    static const int MAX_CIFF_SIZE = 5242880;
    ...
    Pixel getPixel(ull);
    static Ciff parse(istream& , int stage = STAGES, bool checkSoftLimits = true);
